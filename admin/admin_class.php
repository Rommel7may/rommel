...
Class Action {
	...
	function save_user(){
		...
		$type = isset($type) ? $type : 3;
		$data .= ", type = '$type' ";
	
		// Check if username is used by another user
		$chk_sql = "SELECT * FROM users WHERE username = '$username' AND id != $id";
		$chk = $this->db->query($chk_sql)->num_rows;
	
		if ($chk > 0) {
			return 2; // Username already taken by another
		}
	...
	function delete_alumni(){
		...
		// First, get the user ID associated with the alumnus (optional but good for data integrity)
		$user = $this->db->query("SELECT id FROM users WHERE alumnus_id = $id");
		if($user->num_rows > 0){
			$user_id = $user->fetch_assoc()['id'];
			$this->db->query("DELETE FROM users WHERE id = $user_id");
		}
	
		// Now delete from alumnus_bio
	...
	function save_settings(){
		...
		$data .= ", location = '".str_replace("'", "&#x2019;", $location)."' ";
	...
	function save_event(){
		...
		$data = " title = '$title' ";
		$data .= ", schedule = '$schedule' ";
		$data .= ", location = '".htmlentities(str_replace("'", "&#x2019;", $location))."' ";
		$data .= ", content = '".htmlentities(str_replace("'", "&#x2019;", $content))."' ";
		$data .= ", department = '".htmlentities(str_replace("'", "&#x2019;", $department))."' "; // Added department field
